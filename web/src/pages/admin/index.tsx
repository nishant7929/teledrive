...
const Admin: FC<Props> = ({ me, errorMe }) => {
	...
	const {
		...
		error,
		mutate: refetchUsers,
	} = useSWR(params ? `/users?${QueryString.stringify(params)}` : null, fetcher);
	...
	useEffect(() => {
		if (dataConfig?.config) {
			...
			configForm.setFieldsValue({
				...dataConfig.config,
				invitation_code: dataConfig.config.invitation_code
					? `${location.host}/login?code=${dataConfig.config.invitation_code || ''}`
					: null,
			});
		}
	}, [dataConfig]);
	...
	const updateConfig = async() => {
		...
		try {
			...
			return notification.success({
				key: 'update',
				message: 'Updated',
				description: data.config.disable_signup
					? 'Signup is disabled for everyone'
					: data.config.invitation_code
						? 'Signup is enabled by invitation code'
						: 'Signup is enabled for everyone',
			});
		} catch (error: any) {
									...
									<Form.Item name='invitation_code' label='Invitation Code'>
										{dataConfig?.config.invitation_code ? (
											<Input.Search
												...
												enterButton={
													<>
														<ReloadOutlined /> Generate
													</>
												}
												onSearch={() => {
													setLoading(true);
													req.post('/config/resetInvitationCode').then(
														({ data }) => {
															refetchConfig();
															notification.success({
																key: 'update',
							...
							<div style={{ marginTop: '20px' }}>
								<Layout.Content>
									...
									<Popconfirm
										...
										onConfirm={() => {
											Promise.all(
												(selectedRows || [])?.map(async(user: any) => {
													try {
								...
								<Table
									...
									columns={[
										...
										},
										{
											...
											// responsive: ['md'],
											render: (value: any) =>
												moment(value).local().format('llll'),
										},
										{
											title: '',
											dataIndex: 'actions',
											...
											render: (_, record) => (
												<Space>
													<Tooltip
														title={`Switch to ${record.role === 'admin' ? 'user' : 'admin'}`}
													>
														<Button
															...
															onClick={() => {
																req.patch(`/users/${record.id}`, {
																	...
																	.then(() => {
																		notification.success({
																			message: `Switch ${record.username} to ${record.role === 'admin' ? 'user' : 'admin'}`,
																		});
																		refetchUsers();
																	})
																	.catch((error) => {
													...
													<Popconfirm
														...
														title='Are you sure?'
														onConfirm={() => {
															req.delete(`/users/${record.id}`)
																.then(() => {
																	notification.success({
																		message: `Delete ${record.username} successfully!`,
																	});
																	refetchUsers();
																})
																.catch((error) => {

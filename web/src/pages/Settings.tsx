...
const Settings: React.FC<Props> = ({ me, mutate, error }) => {
	...
	const downloadLogs = async() => {
		...
		hiddenElement.href =
			'data:attachment/text,' + encodeURI(sessionStorage.getItem('requests') || '');
		hiddenElement.target = '_blank';
					...
					>
						...
						<Card
							...
							title={
								<Card.Meta
									avatar={
										<Avatar size='large' src={`${apiUrl}/users/me/photo`} />
									}
									title={
										<>
											{me?.user.name}{' '}
						...
						>
							...
							>
								{stats?.stats && (
									<List header='Stats Info' bordered={false}>
										...
										<List.Item key='system'>
											<List.Item.Meta
												...
												description={
													<Tooltip
														title={`Available ${prettyBytes(stats.stats.system.free)}/${prettyBytes(stats.stats.system.size)}`}
													>
														<Progress
															status='active'
															percent={Number(
																(
																	(stats.stats.system.free /
																		stats.stats.system.size) *
																	100
																).toFixed(1),
															)}
														/>
										...
										<List.Item key='cached'>
											<List.Item.Meta
												title='Cached Total Size'
												description={
													<Tooltip
														title={prettyBytes(stats.stats.cachedSize)}
													>
														<Progress
															status='active'
															percent={Number(
																(
																	(stats.stats.cachedSize /
																		stats.stats.system.size) *
																	100
																).toFixed(1),
															)}
								...
								<List header='Interface' bordered={false}>
									{pwa?.canInstall && (
										...
										>
											<List.Item.Meta
												...
												description='Install TeleDrive to your device'
											/>
										</List.Item>
									)}
									...
									<List.Item
										key='expandable-rows'
										actions={[
											<Form.Item name='expandable_rows'>
												<Switch
													onChange={(val) => {
									...
									>
										<List.Item.Meta
											...
											description='Show file details in row table'
										/>
									</List.Item>
								...
								<List header='Operational'>
									{dialogs?.dialogs && (
										<List.Item
											key='saved-location'
											actions={[
												<Form.Item name='saved_location'>
													<Select
														className='saved-location ghost'
														showSearch
													...
													>
														...
														{dialogs?.dialogs
															.filter(
																(d: any) =>
																	d.entity.id != me?.user.tg_id,
															)
															.map((dialog: any) => (
																<Select.Option
																	key={dialog.entity.id}
																	value={buildPathDialog(dialog)}
																>
																	{dialog.title}
																</Select.Option>
															))}
										...
										>
											<List.Item.Meta
												...
												description='Select where to save files'
											/>
										</List.Item>
									)}
									...
									>
										<List.Item.Meta
											...
											description='Reload to checking for updates'
										/>
									</List.Item>
										...
										actions={[
											<Form.Item>
												<Button
													...
													onClick={() =>
														window.open(
															'https://github.com/mgilangjanuar/teledrive/issues/new?assignees=&labels=bug&template=bug_report.md&title=',
															'_blank',
														)
													}
												>
									...
									>
										<List.Item.Meta
											...
											description='Send your activities for reporting'
										/>
									</List.Item>
								</List>
								...
								<List header='Data'>
									...
									>
										<List.Item.Meta
											...
											description='Export your files ref data as JSON'
										/>
									</List.Item>
													...
													<Upload
														...
														beforeUpload={(file) => {
															...
															fileReader.onload = async({
																...
																await req.post('/files/filesSync', {
																	files: JSON.parse(
																...
																notification.success({
																	...
																	description:
																		'Your files has been imported successfully but you need to reshare your files again to update your shared files',
																});
															};
														}}
													>
									...
									>
										<List.Item.Meta
											...
											description='Import your files ref data'
										/>
									</List.Item>
								</List>
								...
								<List header='Danger Zone'>
									...
									>
										<List.Item.Meta
											...
											description='Join to the experimental features'
										/>
									</List.Item>
				...
				<Form form={formRemoval} onFinish={remove} layout='vertical'>
					...
					<Form.Item
						...
						label={
							<Typography.Text>
								Please type{' '}
								<Typography.Text type='danger'>permanently removed</Typography.Text>{' '}
								for your confirmation
							</Typography.Text>
						}
						rules={[{ required: true, message: 'Please input the confirmation' }]}
					>
						<Input />
					</Form.Item>
				</Form>
    ...
    okButtonProps={{ danger: true, type: 'primary', shape: 'round', loading: loadingChangeServer }}>
      ...
      <Typography.Paragraph type="secondary">
        You'll be logged out and redirected to the new server. Please login again to that new server.
			...
			>
				<Typography.Paragraph>Please follow these instructions:</Typography.Paragraph>
				<ol>
					<li>
						Download <a onClick={downloadLogs}>your logs</a>
					</li>
					<li>
						Send an email to <a href={emailLink()}>bug@teledriveapp.com</a> with logs
						and additional screenshots in the attachment
					</li>
				</ol>
			</Modal>
			...
			<Modal
				...
				onOk={() => {
					...
					window.open(
						...
						'_blank',
						// 'location=yes,height=720,width=520,scrollbars=yes,status=yes,top=100,left=300',
					);
				}}
				cancelButtonProps={{ shape: 'round' }}
			...
			>
				...
				<ul>
					<li>
						...
						<Typography.Paragraph>
							Your files will directly upload to the Telegram servers and the speed
							will follow your internet connection.
						</Typography.Paragraph>
					</li>
					<li>
						<strong>Fast Download</strong>
						<Typography.Paragraph>
							Same like Ultra Upload, your files will be downloaded directly from the
							Telegram servers. But, it will have some limitations:
							<ul>
								<li>Only works with chrome-based browsers</li>
								<li>
									The max download size is 2GB for free users or follows your
									device memory
								</li>
							</ul>
						</Typography.Paragraph>
				...
				<Typography.Paragraph>
					Note. Those features may have bugs please report them to{' '}
					<a href={emailLink()}>bug@teledriveapp.com</a> and you can always revoke from
					experimental features anytime.
				</Typography.Paragraph>
